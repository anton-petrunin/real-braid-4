...
Observe that these curves correspond to a new curve $\hat\gamma$ in $\CB^k$ that coincides with $\tilde \gamma$ away from some time interval $[-R,R]$; here we use that $\hat p_i(0)=\hat p_j(0)$ if $\tilde p_i$ and $\tilde p_j$ lie on the left side of $\tilde\lambda$.
