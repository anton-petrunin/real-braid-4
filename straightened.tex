...
This lemma is also a limiting case of the result of Dmitri Burago, Serge Ferleger and Alexey Kononenko on the number of collisions in semi-dispersing billiards \cite{burago-ferleger-kononenko-1997,AKP-invitation};
the proof adapts and gets simpler, but the lemma is not a direct corollary.
...
for $t>0$ and $t<0$ it only rescales and at $t=0$ it approaches a plane angle from both sides; let $\theta$ be its angle measure.
If \ref{1star} does not hold then adding a constant vector to all velocities in the $k$-puncture array can reduce the problem to the case \[(\tilde f\circ\tilde \gamma)'(0-)>0>(\tilde f\circ\tilde \gamma)'(0+).\]
We may still assume that the collision happens at zero at time zero;
so, $\tilde f\circ\tilde \gamma(0)=0$ and from above
\[\tilde f\circ\tilde \gamma(t)\le -\eps\cdot |t|\eqlbl{sharp-max}\]
for some $\eps>0$.
Note that this is impossible if $\theta=\pi$ (in other words, if the ideal points of $\tilde \ell$ are opposite) or if $\tilde \ell$ collapses to one point at the collision.
Assuming that neither case occurs, we will construct a shortcut of $\tilde \gamma$, showing that it is not geodesic.
...
\footnote{If $\tilde\ell(t)$ is simple, then it bounds $L_t$ and $R_t$; the same always holds in a certain generalized sense.}
...
Thus the Busemann functions $x\mapsto |x-\xi|^\flat_t$ can be normalized uniformly.
...
Observe that all modified punctures coincide at $0$; more precisely, if $\tilde p_i$ lies on the left side of $\tilde\lambda$, then $\hat p_i(0)=\delta\cdot u_\xi$.
...
We might have several components of $\tilde\ell(t)$ in the limit.
They must be described by isotopy classes of nonintersecting simple curves with fixed ideal endpoints $\tilde\lambda_1,\ldots,\tilde\lambda_m$;
...
has  a sharp maximum at zero; in other words, $\tilde f_i$ meets \ref{sharp-max}.
...
Finally, note that calculations are local: combining them, one can prove \ref{1star} in the case when several events (collisions and hitting/leaving) happen simultaneously.
\qeds
...
The following observations might save time for someone interested in the conjecture.
...
Given an array of punctures $p_1,\ldots, p_4$, we translate it to make its center of mass zero, then normalize it so that $\mu_1\cdot|p_1|^2+\ldots+ \mu_4\cdot|p_4|^2=1$; it produces a point in $\Sigma^4(\bm{\mu})$.
...
Also, the estimate on $L$, which is described above, might imply that (in the barrier sense) the Hessian of $L$ is nonpositive in some direction, which should prove the conjecture.
