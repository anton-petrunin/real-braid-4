...
We say that $\gamma$ has a \emph{collision} at time $t$ if at this time the velocities of some $p_i$ change;
...
%If $p_1(t)=\ldots=p_n(t)$ for some $t$, then $\gamma$ might have a collision only at time $t$.
...
a collision happens when geodesic crosses certain half-lines from the vertex of the angle.
This observation reduces the question to finding a universal bound for the number of collisions of geodesic $\gamma$ that has length at most $\eps$ and such that $\bar\gamma$ lies at the distance at least $1$ from the diagonal line. Indeed, we can choose finitely many such segments, with their number bounded in terms of $varepsilon$ alone, so that the cones over them cover the whole cone over $\gamma$.
...
(In particular, if all punctures are distinct, then the point has a flat neighborhood.)
Therefore, if $\eps>0$ is small, then there is a universal bound on number of collisions for all geodesics $\gamma$ described above, and this implies the lemma.
\qeds
...
A point $\bm{p}\in\WW^n$ will be regarded as an array of $n$ distinct punctures $(p_1,\dots,p_n)$ of the complex plane $\CC$.
...
\footnote{If $\tilde\ell(t)$ is simple, then it bounds $L_t$ and $R_t$; the same always holds in an obvious generalized sense.}
...
\footnote{Equivalently, we can apply a diagonal matrix with $\sqrt{\mu_1},\dots,\sqrt{\mu_n}$ on the diagonal to the complex hyperplane arrangement $z_i=z_j$ (so the new complex arrangement is defined by $\frac{z_i}{\sqrt{\mu_i}}=\frac{z_j}{\sqrt{\mu_j}}$).
...
Given an array of punctures $p_1,\dots, p_4$, we have to move it making to make its center of mass zero, then normalize so that $\mu_1\cdot|p_1|^2+\dots+ \mu_4\cdot|p_4|^2=1$; it produces a point in $\Sigma^4(\bm{\mu})$.
