...
Consider the Busemann function $b_t\:L_t\to\RR$ for $\xi$ and normalize them so that $b_t(t\cdot x)=t\cdot b_1(x)$ for any $t\ge 0$ and $x\in L_1$.
Note that superlevel sets of $b_t$ are compact.
Further, $b_t(x)\le -\eps\cdot|t|$ for any $x\in L_t$ and some fixed $\eps>0$; otherwise we do not have a strict maximum at zero.
(The maximum of $b_t(x)$ for $x\in L_t$ gives a lower bound for $\tilde f\circ\tilde \gamma(t)$.)
