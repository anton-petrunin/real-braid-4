...
where $x=p_3^{-1}\cdot p_2^{-1}\cdot d\cdot p_2\cdot p_3$, $y=p_3^{-1}\cdot d\cdot p_3$, and $z=d$ are elements of the free group $F_2=\ker\phi=\llangle d\rrangle$, and $\phi$ is the homomorphism $B_4\to B_3$ that identifies $a$ and $b$.
...
Otherwise $p_3\in\langle a\rangle$, and the statent follows.
...
\[p_1\cdot d^{\eps_1}\cdot p_2 = d^{-N}\cdot p_3^{-1}\cdot d^{-\eps_2}.\]
...
Note that $p_3=a^r\cdot\zeta^k$ and $p_1=a^q\cdot \zeta^{-k}$.
Therefore
\[
...
\qquad \epsilon=\pm1.
\]
Thus, $p_2\in\langle a\rangle\cdot \langle c\rangle\cdot \langle a\rangle$, and the proof is finished.
\qeds
