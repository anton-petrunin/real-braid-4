...
\begin{thm}{Lemma}
Suppose
\[p_1\cdot d^{n_1}\cdot p_2 \cdot d^{n_2}\cdot  p_3\cdot  d^{n_3}=1,\]
where $p_i\in \langle a,c\rangle$ and $n_i\ne 0$.
Then $p_i\in\langle a\rangle\cdot \langle c\rangle\cdot \langle a\rangle$ for some $i$.
\end{thm}

This equation can be rewritten as
\[p_1\cdot p_2\cdot p_3\cdot x^{n_1}\cdot y^{n_2}\cdot z^{n_3}=1,\]
where $x=p_3^{-1}\cdot p_2^{-1}\cdot d\cdot p_2\cdot p_3$, $y=p_3^{-1}\cdot d\cdot p_3$, and $z=d$ are elements of the free group $F_2=\ker\phi$, and $\phi$ is the homomorphism $B_4\to B_3$ that identifies $a$ and $b$.

Note that $p_1\cdot p_2\cdot p_3=1$ and $x^{n_1}\cdot y^{n_2}\cdot z^{n_3}=1$.
...
Put $\epsilon_i=n_i\in\{\pm1\}$, $i=1,2$, and $N=n_3$.
The curve argument gives
\[
p_1\cdot \langle a\rangle=a^{-N}\cdot p_3^{-1}\cdot \langle a\rangle,
\]
so, for some $u\in\mathbb Z$, we have
\[
p_1=a^{-N}\cdot p_3^{-1}\cdot a^u
\quad\text{and}\quad
...
Hence $p_3\in\langle a\rangle\cdot Z(B_3)$.
Writing $p_3=a^r\cdot\zeta^k$, the above expression for $p_1$ gives
$p_1=a^q\cdot \zeta^{-k}$, and therefore
\[
p_2=p_1^{-1}\cdot p_3^{-1}\in\langle a\rangle.
\]
...
\[
p_3=q\cdot \zeta^{2\cdot k},\qquad
q=a^r\cdot  c^{\epsilon_1}\cdot a^t.
\]
Consequently
\[
p_2
=a^{-u}\cdot p_3\cdot a^N\cdot p_3^{-1}
=a^{-u}\cdot q\cdot a^N\cdot q^{-1}
...
$\langle a\rangle\cdot \langle c\rangle\cdot \langle a\rangle$.
