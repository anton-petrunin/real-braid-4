...
This step is possible since $\Sigma$ admits a cocampact action by $B_4$ and so we can apply the mimiting procedure. %???
Therfore our question reduces to the following claim which we are going to prove in the following sections.
...
We will be interested in trajectories that locally minimize their length; these are possible candidates for projections of closed geodesics in $\RB^4$ to $\triangle$.

Such a trajectory $\gamma$ meets the following properties:
\begin{itemize}
\item If a vertex lies in an interior of a side, then $\gamma$ behaves as a billiard trajectory; that is, the angle of incidence is equal to the angle of reflection.
...
Following $\gamma$ we get a word from $a$, $b$, and $c$ with some powers which gives, which product is $1$ in $B_4$ (since $\gamma$ is closed in $\RB^4$).
...
=\varnothing.
\]
This case follows from the first Agol's lemma.
