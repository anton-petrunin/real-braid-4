...
\section{Agol's lemmas}

\begin{thm}{First lemma}
...
The following proof is taken almost verbatum from  the answer by Ian Agol to our question \cite{agol}.
...
Then in the isotopy class of $w$ there is a unique geodesic arc.
...
Put $s=h_1\cdot k_1=k_2^{-1}\cdot h_2^{-1}$; note that
...
Consider the subdisc $M\subset \DD^2$ that is the middle piese of $\DD^2$ after cutting along $x\cup y$.
Note that $M\ni p_2, p_3$, so $M\setminus \{p_2,p_3\}$ is a twice-punctured disk, and it contains only one isotopy class of essential arcs with endpoints on the boundary.
Then each component of $s(x)\cap M$ and $s(y)\cap M$ must be isotopic to this arc.
All components of $s(x)\cap M$ must have endpoints on $x\subset \partial M$,
and all components of $s(y) \cap M$ must have endpoints on $y\subset M$.
By compsing $s$ with an element of $\langle c\rangle$, we can unwind this configuration;
...
Now let us focus on the subdiscs $L$ and $R$ in $\DD^2$ and obtained by cutting along $z$ and taking the left and rught piece, respectively.
